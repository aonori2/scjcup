...
function contentboxFunc($atts , $content = null) {
	if($atts && $content) {
		...
		if(!$title && $class) {
			return '<div class="c_box ' . $class . $type .'">' . do_shortcode($content) . '</div>';

		} elseif($title && $class) {
			return '<div class="c_box intitle ' . $class . $type .'"><div class="box_title"><span class="span__box_title">' . $title . '</span></div>'. do_shortcode($content) .'</div>';
		}
	}
}
